...
async def get_token_async(session, num, pwd):
    ...
    data = {'grant_type': 'password', 'username': num, 'password': pwd, 'client_id': 'ana-vodafone-app', 'client_secret': '95fd95fb-7489-4958-8ae6-d31a525cd20a'}
    try:
...
def get_token(num, pwd):
    url = "https://mobile.vodafone.com.eg/auth/realms/vf-realm/protocol/openid-connect/token"
    data = {'username': num, 'password': pwd, 'grant_type': 'password', 'client_id': 'my-vodafone-app', 'client_secret': 'a2ec6fff-0b7f-4aa4-a733-96ceae5c84c3'}
    try:
...
async def add_member_async(session, token, owner, member, quota):
    ...
    headers = {'Authorization': f"Bearer {token}", 'msisdn': owner, 'Content-Type': "application/json", 'clientId': "WebsiteConsumer"}
    try:
...
async def accept_invitation_async(session, owner, member, m_token):
    ...
    data = {"category": [{"listHierarchyId": "TemplateID", "value": "47"}], "name": "FlexFamily", "parts": {"member": [{"id": [{"schemeName": "MSISDN", "value": owner}], "type": "Owner"}, {"id": [{"schemeName": "MSISDN", "value": member}], "type": "Member"}]}, "type": "AcceptInvitation"}
...
def run_money_back_scan(num, token):
    ...
    headers = {'Authorization': f'Bearer {token}', 'msisdn': num}
    try:
...
def execute_flex_discount(num, token, pkg_key):
    ...
    payload = {"channel": {"name": "MobileApp"}, "orderItem": [{"action": "add", "id": pkg['product_id'], "product": {"characteristic": [{"name": "TariffID", "value": pkg['tariff_id']}]}, "@type": "Access fees Discount"}], "@type": "InterventionTariff"}
    try:
