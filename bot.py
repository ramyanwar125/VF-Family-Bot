...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "يرجى اختيار القسم الذي ترغب في استخدامه: 👇"
    )
...
async def menu_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'MB':
        ...
        kb = [[InlineKeyboardButton("➕ إضافة عضو", callback_data='F_ADD'), InlineKeyboardButton("✅ قبول دعوة", callback_data='F_ACC')], 
              ...
              [InlineKeyboardButton("🔙 العودة للقائمة", callback_data='BACK')]]
        await query.edit_message_text("👥 **إدارة مجموعة العائلة**", reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
        return FAM_SUB
        ...
        await query.edit_message_text("🎁 **قسم Flex Discount**\n\nاختر الباقة المراد تفعيل الخصم عليها: 👇", reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
        return SELECT_FINAL
...
async def get_num(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🔑 أرسل **كلمة المرور** الحالية:")
    return GET_PWD
...
async def get_m_num(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if op in ['F_ADD', 'F_AUTO']:
        kb = [[InlineKeyboardButton("1300 (10%)", callback_data='1300')], 
              [InlineKeyboardButton("2600 (20%)", callback_data='2600')], 
              [InlineKeyboardButton("5200 (40%)", callback_data='5200')]]
        await update.message.reply_text("📊 **اختر نسبة توزيع الفليكسات:**", reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
        return GET_QUOTA
    elif op == 'F_ACC':
        await update.message.reply_text("🔑 أرسل **باسورد العضو** لإتمام القبول:")
        return GET_M_PWD
    return await run_process(update, context)
...
async def run_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if op == 'MB_SCAN':
            ...
            await status.edit_text(f"💰 رصيد الماني باك: `{res}` ج.م", parse_mode='Markdown')
        elif op == 'F_OFFER':
            res = await asyncio.to_thread(engine.execute_order, ud['num'], token, ud['selected_pkg'], 'FLEX')
            await status.edit_text("✅ **تم تفعيل Flex Discount بنجاح!**" if res else "❌ **الفشل: الخط غير مؤهل.**")
        # يمكن إضافة باقي عمليات العائلة هنا بنفس المنطق
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            GET_M_NUM: [MessageHandler(filters.TEXT & ~filters.COMMAND, get_m_num)], # تأكد من وجودها هنا
