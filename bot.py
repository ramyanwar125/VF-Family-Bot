...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "✨ **أهلاً بك في نظام فودافون المتكامل** ✨\nيرجى اختيار القسم:"
...
async def menu_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'MB':
        ...
        await query.edit_message_text("🎁 **اختر باقة Flex Discount:**", reply_markup=InlineKeyboardMarkup(kb))
        return SELECT_FINAL
    ...
    try:
        ...
        if op == 'MB_SCAN':
            ...
            if await asyncio.to_thread(engine.family_op, ud['num'], token, ud['m_num'], 'SEND', ud['quota']):
                ...
                mt = await asyncio.to_thread(engine.get_token, ud['m_num'], ud['m_pwd'])
                res = await asyncio.to_thread(engine.family_op, ud['num'], None, ud['m_num'], 'ACCEPT', m_token=mt)
                await status.edit_text("🚀 تم التفعيل التلقائي!" if res else "⚠️ تم الإرسال وفشل القبول")
        elif op == 'F_REM':
            res = await asyncio.to_thread(engine.family_op, ud['num'], token, ud['m_num'], 'REMOVE')
            await status.edit_text("✅ تم حذف العضو" if res else "❌ فشل")
...
async def final_exe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "X_FLX_" in query.data:
        ...
        await query.edit_message_text("✅ تم الاسترداد بنجاح" if res else "❌ فشل")
        return await start(update, context)
