...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    kb = [
        [InlineKeyboardButton("💰 Money Back", callback_data='MB')],
...
async def menu_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'MB':
        ...
        await query.edit_message_text("🎁 **قسم Flex Discount**\nاختر الباقة لتفعيل الخصم:", reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
        return SELECT_FINAL
...
async def get_m_num(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if op in ['F_ADD', 'F_AUTO']:
        kb = [[InlineKeyboardButton("10%", callback_data='10'), 
               InlineKeyboardButton("20%", callback_data='20'), 
               InlineKeyboardButton("40%", callback_data='40')]]
        await update.message.reply_text("📊 **اختر نسبة التوزيع:**", reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
        return GET_QUOTA
    elif op == 'F_ACC':
        await update.message.reply_text("🔑 أرسل **كلمة مرور العضو** للقبول:")
        return GET_M_PWD
    return await run_process(update, context)
...
async def handle_extra(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.callback_query:
        ...
        if context.user_data['op'] == 'F_AUTO':
            await update.callback_query.edit_message_text("🔑 أرسل **كلمة مرور العضو** للتفعيل التلقائي:")
            return GET_M_PWD
    else:
...
async def run_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status = await msg.reply_text("⏳ **جاري المعالجة...**", parse_mode='Markdown')
